        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if _should_fallback and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    # to fallback.
                    #
                    # Upstream-aggregator 429 is another exception — the
                    # credential pool can't help when the *upstream* model
                    # (DeepSeek, etc.) is throttling OpenRouter, so always fall
                    # back to a different model regardless of pool state.
