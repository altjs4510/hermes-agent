...
# ---------------------------------------------------------------------------
# Stale cleanup (monthly) — expire long-untouched pending proposals
# ---------------------------------------------------------------------------

_STALE_MAX_AGE_DAYS = 90
_STALE_SCRIPT_NAME = "self_improvement_stale_cleanup.py"
_STALE_CLEANUP_JOB_NAME = "self-improvement-stale-cleanup"

_STALE_SCRIPT = '''#!/usr/bin/env python3
"""Auto-generated by tools/self_improvement_tool.setup_stale_cleanup_cron.
Monthly cleanup: expire self-improvement proposals left pending > 90 days."""
from tools.self_improvement_tool import cleanup_stale_proposals

r = cleanup_stale_proposals()
if r.get("expired"):
    print(f"🧹 자가발전 미처리 제안 {r['expired']}건이 {r['max_age_days']}일 경과로 정리(만료)됐어.")
'''
...
def cleanup_stale_proposals(
    ...
    """Expire self-improvement proposals stuck in ``proposed`` past max_age_days.

    Keeps the owner-confirm store from accumulating forgotten pending cards.
    Only proposals whose latest state is still ``proposed`` are touched;
    confirmed/executed/rejected/expired are left alone. Returns
    ``{"expired", "ids", "max_age_days"}``.
    """
    store_path = Path(_owner_confirm_store_path())
    if not store_path.exists():
        return {"expired": 0, "ids": [], "max_age_days": max_age_days}
    ...
    for line in store_path.read_text(encoding="utf-8").splitlines():
        ...
        pid = ev.get("proposal_id")
        if pid:
            latest[pid] = ev  # append-ordered → last wins
    ...
    for ev in latest.values():
        if ev.get("action_class") != _ACTION_CLASS or ev.get("state") != "proposed":
            continue
        try:
            created = datetime.fromisoformat(str(ev.get("ts", "")).replace("Z", "+00:00"))
        except Exception:
            continue
        if created < cutoff:
            try:
                store.expire(proposal_id=str(ev["proposal_id"]), now=now)
                expired.append(str(ev["proposal_id"]))
            except Exception as e:  # pragma: no cover - defensive
                logger.warning("stale cleanup: expire failed for %s: %s", ev.get("proposal_id"), e)
    if expired:
        logger.info("self_improvement stale cleanup: expired %d proposal(s)", len(expired))
    return {"expired": len(expired), "ids": expired, "max_age_days": max_age_days}


def setup_stale_cleanup_cron(
    owner_id: Optional[str] = None,
    schedule: str = "0 9 1 * *",  # 1st of month, 09:00
    scripts_dir: Optional[os.PathLike[str] | str] = None,
    force: bool = False,
) -> Dict[str, Any]:
    """Install the monthly stale-cleanup cron (script + job). Idempotent.

    Delivers a one-line note to the owner DM only when something was expired
    (silent otherwise).
    """
    owner_id = owner_id or (_owner_ids()[0] if _owner_ids() else "")
    if not owner_id:
    ...
    sd = Path(scripts_dir) if scripts_dir else _SCRIPTS_DIR
    sd.mkdir(parents=True, exist_ok=True)
    script_path = sd / _STALE_SCRIPT_NAME
    script_path.write_text(_STALE_SCRIPT, encoding="utf-8")

    from cron.jobs import create_job, load_jobs

    existing = [j for j in load_jobs() if j.get("name") == _STALE_CLEANUP_JOB_NAME]
    if existing and not force:
        return {"status": "exists", "job_id": existing[0].get("id"), "script": str(script_path)}
    ...
    job = create_job(
        prompt=None,
        schedule=schedule,
        name=_STALE_CLEANUP_JOB_NAME,
        script=_STALE_SCRIPT_NAME,
        no_agent=True,
        deliver="origin",
        origin={"platform": "slack", "chat_id": owner_id},
    )
