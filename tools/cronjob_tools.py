...
def _validate_enabled_toolsets(names: Optional[List[str]]) -> Optional[str]:
    ...
    Otherwise a hallucinated name (e.g. ``mcp_ms365`` for the real ``mcp-ms365-mcp`` /
    alias ``ms365-mcp``) is silently dropped by model_tools' resolution loop and the job
    runs without the intended tools. Returns an error string if any name is unknown."""
    if not names:
        return None
    try:
        from toolsets import validate_toolset, get_toolset_names
    except Exception:
        return None  # resolution still warns at runtime
    unknown = [n for n in names if not validate_toolset(n)]
    if not unknown:
        return None
    return (
        f"Unknown toolset name(s): {', '.join(repr(n) for n in unknown)}. "
        f"To enable an MCP server's tools use 'mcp-<server>' (e.g. 'mcp-ms365-mcp'), not 'mcp_<server>'. "
        f"Available toolsets: {', '.join(sorted(get_toolset_names()))}")
...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    error = (
        ...
        or _validate_enabled_toolsets(a["enabled_toolsets"]))
    if error:
...
def _update_run_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if a["enabled_toolsets"] is not None:
        ts_error = _validate_enabled_toolsets(a["enabled_toolsets"])
        if ts_error:
            return ts_error
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "enabled_toolsets": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Optional toolset names to restrict the job's agent to (e.g. [\"web\", \"terminal\"]) — cuts token overhead. Infer from the prompt. Omit for all default tools. For an MCP server use \"mcp-<server>\" (or the bare server name), NOT \"mcp_<server>\"; unknown names are rejected. On update, [] clears."
            },
            "workdir": {
                "type": "string",
