...
def request_gateway_approval(
    *,
    ...
    The side-effect/tool analogue of ``check_all_command_guards``: it goes through
    the same per-session queue, Slack/Discord/etc. button UI, coalescing, timeout
    contract and ``resolve_gateway_approval`` path (``_await_gateway_decision``),
    except the caller supplies the risk description instead of relying on
    shell-command pattern detection. Use it for non-shell shared side effects such
    as sending a Slack DM from a Slack-originated session.
    """
    if _YOLO_MODE_FROZEN or is_current_session_yolo_enabled() or approval_context._get_approval_mode() == "off":
