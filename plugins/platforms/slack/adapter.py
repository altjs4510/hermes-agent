...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slack_message_impl(
        ...
        is_bot_message = self._event_declares_bot_sender(event)
        if not is_bot_message and msg_user and not event.get("client_msg_id"):
            is_bot_message = await self._resolve_user_is_bot(
                msg_user,
        ...
        if slack_attachments:
            ...
            for att in slack_attachments:
                ...
                # cookie.alter parity: message-share unfurls (is_msg_unfurl) are
                # kept, not skipped — a quoted/forwarded Slack message is exactly
                # the content a request like "아래 내용 보고 처리해" refers to.
        ...
        # claim belongs here — after every filter has passed (including the
        # cookie.alter multi-actor/thread-heuristic yields above), before the
        # slow enrichment awaits that open the race.
        ...
        # Reuse the routing-time prefetch (cookie.alter parity, above) instead
        # of re-fetching when it already warmed the cache for this thread.
        ...
        # channel_context hydrates the root separately (see
        # bac960e23d "fix(slack): stop injecting thread roots as reply
        # context" — reply_to_text is intentionally not populated here).
