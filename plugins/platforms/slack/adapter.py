...
_SLACK_PERMALINK_STRIP_RE = re.compile(
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    canonical = _SLACK_PERMALINK_STRIP_RE.sub(r"\1", canonical)
...
async def _cancel_orphaned_client_tasks(client: Any) -> int:
    ...
    Production incident 2026-08-20/21: ~6 errors/min for roughly two hours.

    Upstream tracks the same bug in NousResearch/hermes-agent#46990 (root cause:
    slackapi/python-slack-sdk#1913).

    ⚠️ CONVERGENCE PLAN (2026-08-24, re-checked 2026-09-13 during the upstream
    merge): the original plan was to drop this sweep once upstream PR #83693
    (re-read the client task attributes for up to 3 passes after
    ``close_async()``) merged. **That is still NOT met.** Upstream's current
    ``_stop_socket_mode_handler`` instead cancels every ``connect()``-reachable
    task *before* ``close_async()``, which does nothing for a task rebound
    *during* ``close()`` -- exactly the race here. Dropping the sweep on the
    strength of that pre-close cancellation makes
    ``test_slack_socket_reconnect_heal.py::TestSocketModeTeardown::
    test_task_rebound_during_close_does_not_survive`` fail. So this stays until
    upstream covers the rebind-during-close window specifically; keep upstream's
    pre-close cancellation, this sweep is additive and runs after
    ``close_async()``. Our regression test for the rebind window is offered
    upstream as RelaxJonh/hermes-agent#1.
    """
    if client is None:
        return 0
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _stop_socket_mode_handler(self) -> None:
        ...
        # pre-close cancellation above can miss a task created partway through
        # teardown. Sweep once more by walking the live task set: anything still
        # bound to this client is an orphan that would retry against the closed
        # session forever. See _cancel_orphaned_client_tasks.
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        # Owner-confirm buttons: the user-facing UX for proposed side effects
        # (text approval stays a fallback/debug path).
        for _action_id in ("hermes_owner_confirm_approve", "hermes_owner_confirm_cancel"):
            self._app.action(_action_id)(self._handle_owner_confirm_action)
        # Message shortcut "쿠키 위키에 저장" (callback_id cookie_wiki_save). Registered only when
        # the app object supports shortcuts — minimal test doubles don't expose ``.shortcut``, and
        # a missing optional handler must not fail the whole connection.
        if hasattr(self._app, "shortcut"):
            self._app.shortcut("cookie_wiki_save")(self._handle_wiki_save_shortcut)
    ...
    async def _send_owner_confirm_proposal(
        ...
        *,
        ...
        async def _execute_send(_proposal: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if sent_ts:
                # Workspace-scoped markers: bare ts values no longer match the wake check.
                _team_id = self._metadata_team_id(metadata)
                self._bot_message_ts.add(self._workspace_message_marker(_team_id, sent_ts))
                if thread_ts:
                    self._bot_message_ts.add(self._workspace_message_marker(_team_id, thread_ts))
            return {
    ...
    async def send(
        ...
        try:
            ...
            # Side-effect delegation gate: a send flagged for owner-confirm posts a
            # preview + approval card instead of delivering (gateway.owner_confirm).
            ...
            if isinstance(owner_confirm, dict) and owner_confirm.get("require"):
                return await self._send_owner_confirm_proposal(
                    ...
                    metadata=metadata or {}, owner_confirm=owner_confirm)
    ...
    async def _channel_gate_allows(
        ...
        if not free_channel and not bot_uid:
            # Fail closed when mention gating is on but auth/team metadata has not resolved our
            # bot user ID: otherwise every channel message falls through as if meant for Hermes.
            logger.warning(
                "[Slack] Ignoring channel message because bot user ID is unknown while "
                "require_mention=true (channel=%s team=%s)", channel_id, team_id)
            return False
    ...
    def _slack_free_response_channel(self, channel_id: str) -> bool:
        """True when this channel answers without a mention (free-response, or mention gating off).
        ``require_mention_channels`` overrides both."""
    ...
    async def _owner_one_to_one_thread(
        self, *, channel_id: str, event_thread_ts, team_id: str, is_thread_reply: bool,
        is_bot_message: bool) -> bool:
        """True when an un-@mentioned reply is still addressed to us: a single-human thread
        (owner ↔ bot 1:1), where nobody re-@mentions the bot on every turn of a back-and-forth.

        Everything else (multiple humans, or an unresolved participant count) routes through the
        NO_REPLY gate instead of going silent: the agent answers when the message is clearly for it
        and emits NO_REPLY — which still leaves 👀 — for human↔human chatter. Treating those as
        un-addressed used to swallow direct instructions like "응 진행".

        A bot sender is never "the owner in a 1:1": its posts always go through the NO_REPLY gate,
        or two bots ack each other forever (2026-09-10: cookie.alter acked every Kitty.Alter post).
        """
        if not (self._slack_alter_thread_heuristic() and is_thread_reply) or is_bot_message:
            return False
        human_count = await self._thread_human_participant_count(
            channel_id=channel_id, thread_ts=event_thread_ts or "", team_id=team_id)
        return human_count is not None and human_count <= 1
    ...
    def _append_link_unfurls(text: str, slack_attachments: list) -> str:
        """Append link-unfurl previews (``attachments``) to ``text``. Dedup matches the rendered
        section, not the bare URL (which is usually already in the user's text while the preview
        body is not).

        cookie.alter parity: message-share unfurls (``is_msg_unfurl``) are kept, not skipped — a
        quoted/forwarded Slack message is exactly the content a request like "아래 내용 보고
        처리해" refers to. (The hydration path, :meth:`_render_message_text`, still skips them:
        there the unfurl echoes a message the thread already carries.)"""
        att_parts: list[str] = []
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        # Owner-confirm approvals ("승인 …") are resolved here and never reach the agent loop.
        if await self._handle_owner_confirm_message(
                text=original_text, channel_id=channel_id, thread_ts=str(thread_ts or ""),
                message_ts=ts, user_id=user_id):
            return
        bot_uid = self._team_bot_user_ids.get(team_id, self._bot_user_id)
        ...
        if await self._peer_bot_drop(event, user_id, bot_uid, channel_id, team_id, is_mentioned):
            return
        is_bot_message = self._event_declares_bot_sender(event) or bool(
            user_id and user_id != bot_uid
            and await self._resolve_user_is_bot(user_id, chat_id=channel_id, team_id=team_id))
        # Set when the message reached us through thread membership rather than a direct
        # @mention — the agent then decides reply-vs-NO_REPLY semantically.
        unaddressed_thread_reply = False
        if (not is_dm and is_thread_reply and event_thread_ts
                and self._slack_prefetch_thread_context_for_routing()):
            # cookie.alter parity: read the root + prior replies BEFORE deciding respond vs
            # react. Also warms the per-thread cache for participant counting and the later
            # context injection, so the common path makes no extra conversations.replies call.
            await self._fetch_thread_context(
                channel_id=channel_id, thread_ts=event_thread_ts, current_ts=ts, team_id=team_id)
        if not is_one_to_one_dm:
            if not await self._channel_gate_allows(
                    channel_id=channel_id, routing_text=routing_text, bot_uid=bot_uid,
                    is_mentioned=is_mentioned, is_thread_reply=is_thread_reply,
                    event_thread_ts=event_thread_ts, user_id=user_id, team_id=team_id,
                    is_dm=is_dm, force_process=force_process):
                return
            if (not force_process and not is_mentioned
                    and not self._slack_free_response_channel(channel_id)):
                # Woke through thread membership/session, not a direct mention.
                #
                # Multi-actor yield: the thread is ours, but this message @-mentions someone
                # else, so the turn is addressed to that actor. is_mentioned is False here, so
                # any <@…> token is necessarily a *different* actor (broadcasts use <!…>). Ack
                # with 👀 and wait for a fresh direct mention. Author-agnostic: a human or a bot
                # naming a third party is the same signal — "not your turn."
                if re.search(r"<@[A-Z0-9]+>", routing_text):
                    if self._reactions_enabled():
                        await self._add_reaction(channel_id, ts, "eyes", team_id)
                    logger.debug(
                        "[Slack] yielding: thread %s message @-mentions another actor, not "
                        "this bot (%s)", event_thread_ts, bot_uid)
                    return
                unaddressed_thread_reply = not await self._owner_one_to_one_thread(
                    channel_id=channel_id, event_thread_ts=event_thread_ts, team_id=team_id,
                    is_thread_reply=is_thread_reply, is_bot_message=is_bot_message)
        ...
        if is_mentioned:
            text, original_text, command_probe_text, is_command_text = self._apply_bot_mention(
                ...
                team_id)
        # cookie.alter parity: read the body of Slack messages linked by permalink, not just the
        # preview Slack's own unfurl carries ("아래 링크 본문 보고 처리해").
        linked_context = await self._fetch_linked_slack_message_context(
            text, current_channel_id=channel_id, team_id=team_id)
        if linked_context:
        ...
        (
            ...
            is_dm=is_dm)
        # Resolve <@U...> to <@U...|name> so the agent sees who was pointed at without
        # reverse-resolving raw IDs. Idempotent and skips our own mention, so the routing
        # decisions above (which ran on the raw text) are unaffected.
        ...
        msg_event = await self._build_message_event(
            ...
            unaddressed_thread_reply=unaddressed_thread_reply)
    ...
    async def _build_message_event(
        ...
        channel_context: Optional[str], unaddressed_thread_reply: bool = False) -> MessageEvent:
        ...
        # Un-addressed thread message: append the NO_REPLY directive as an ephemeral channel
        # prompt (applied at API time, never persisted) so the agent can stay quiet on small
        # talk / asides while still answering when a reply is genuinely useful.
        channel_prompt = self._channel_prompt_with_identity(channel_id, team_id)
        if unaddressed_thread_reply:
            directive = self._no_reply_thread_directive()
            channel_prompt = f"{channel_prompt}\n\n{directive}" if channel_prompt else directive
    ...
    async def _inject_canvas_text(
        self, f: Dict[str, Any], url: str, channel_id: str, team_id: str, text: str,
        notices: List[str]) -> str:
        """Prepend a Slack canvas body (as markdown) to ``text``; unreadable canvases only
        add an attachment notice."""
        try:
            canvas_md = await self._fetch_slack_canvas_markdown(url, team_id=team_id)
        except Exception as e:  # pragma: no cover - defensive logging
            detail = self._describe_slack_download_failure(
                e, file_obj=f) or self._describe_slack_api_error(
                    getattr(e, "response", None), file_obj=f)
            self._note_attachment_failure(
                notices, detail, "[Slack] Failed to read canvas %s: %s", f.get("id"), e,
                exc_info=True)
            return text
        if not canvas_md:
            return text
        # Canvas mentions arrive as bare "@U…"; normalize so _enrich_mentions can resolve them.
        canvas_md = await self._enrich_mentions(
            re.sub(r"@(U[A-Z0-9]{6,})\b", r"<@\1>", canvas_md), chat_id=channel_id, team_id=team_id)
        title = re.sub(r"[^\w.\-:&<>|@() ]", " ", f.get("title") or f.get("name") or "Canvas").strip()
        injection = f"[Slack Canvas: {title}]\n{canvas_md}"
    ...
    async def _collect_inbound_media(
        ...
        for f in event.get("files", []):
            ...
            if not url:
                continue
            if mimetype == "application/vnd.slack-docs" or f.get("filetype") == "quip":
                # Slack canvas (quip doc): ``url_private`` serves the rendered HTML body, so
                # convert it to markdown and inject — otherwise the agent sees only the file ID.
                text = await self._inject_canvas_text(
                    f, f.get("url_private") or url, channel_id, team_id, text, notices)
                continue
    ...
    async def _handle_wiki_save_shortcut(self, ack, shortcut, client) -> None:
        """Message shortcut "쿠키 위키에 저장": add a row to the same List the native "리스트에
        추가" writes to (permalink in the Message column), then enrich it immediately.

        The native path is picked up by a 2-minute cron; the shortcut runs the enricher detached
        right away (never on the event loop) so the row is tidied within seconds.
        """
        await ack()
        try:
                ...
                if channel and user_id:
                    try:
                        await client.chat_postEphemeral(
                            channel=channel, user=user_id, text="이건 쿠키 전용 기능이에요. (위키 저장)")
                    except Exception:
                        pass
            ...
            permalink = f"https://ffcoit.slack.com/archives/{channel}/p{ts.replace('.', '')}"
            if tts:
                permalink += f"?thread_ts={tts}&cid={channel}"
            list_id = os.getenv("COOKIE_WIKI_LIST_ID", "F0B9TUCMFK2")
            # The link goes in the native Message column (same as a native add): Slack parses the
            # permalink into channel/ts/thread itself.
            ...
            if channel and user_id:
                try:
                    await client.chat_postEphemeral(
                        channel=channel, user=user_id, text="🔖 위키에 저장했어요 — 주제 정리 중 (몇 초).")
                except Exception:
                    pass
        except Exception as e:  # pragma: no cover - defensive
    ...
    async def _fetch_thread_context(
        ...
        try:
            ...
            bot_uid = self._team_bot_user_ids.get(team_id, self._bot_user_id)
            human_count = len({
                msg.get("user", "") for msg in messages
                if msg.get("user") and msg.get("user") != bot_uid
                and not (bool(msg.get("bot_id")) or msg.get("subtype") == "bot_message")})
    ...
    async def _format_thread_context(
        ...
        for msg in messages:
            ...
            # cookie.alter parity: read the actual body of Slack messages
            # explicitly linked (permalink) from within this thread message,
            # not just the preview Slack's own unfurl provides.
            linked_context = await self._fetch_linked_slack_message_context(
                msg_text,
    ...
    async def _fetch_slack_canvas_markdown(self, url: str, team_id: str = "") -> str:
        ...
        Canvases legitimately serve ``text/html`` from ``url_private`` (the rendered doc), so —
        unlike :meth:`_download_slack_file_bytes` — HTML is converted here instead of rejected.
        The bot token still rides along, so the same SSRF + Slack-CDN allowlist applies.
        """
        import httpx
        from tools.url_safety import create_ssrf_safe_async_client, is_safe_url
        if not is_safe_url(url) or not self._is_slack_cdn_url(url):
            raise ValueError(
                f"Blocked unsafe Slack canvas URL: {safe_url_for_log(url)}")
        bot_token = self._resolve_download_token(url, team_id)
        async with create_ssrf_safe_async_client(
            timeout=30.0, follow_redirects=True, event_hooks={"response": [_ssrf_redirect_guard]}
        ) as client:
            for attempt in range(3):
                try:
                    ...
                    if attempt >= 2:
                        raise
                    logger.debug(
                        "Slack canvas fetch retry %d/2 for %s: %s", attempt + 1, url[:80], exc)
                    await asyncio.sleep(1.5 * (attempt + 1))
        return ""
