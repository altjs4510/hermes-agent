...
def test_rebuilds_reapply_the_non_owner_access_gate(monkeypatch):
    """A non-owner agent's gate (agent_init) must survive MCP refresh and session prefix restore:
    neither path may re-expose a tool the gate dropped at build time."""
    import model_tools
    from tools import registry as registry_mod

    agent = _agent(["read_file"])
    agent._access_gate_blocked = lambda name: name in {"terminal", "mcp_ms365_create_event"}
    served = [_tool(n) for n in ("read_file", "terminal", "mcp_ms365_create_event", "mcp_ms365_list_events")]
    monkeypatch.setattr(model_tools, "get_tool_definitions", lambda **kw: list(served))
    entries = {d["function"]["name"]: types.SimpleNamespace(name=d["function"]["name"], schema=d["function"]) for d in served}
    monkeypatch.setattr(registry_mod.registry, "get_all_entries", lambda: list(entries.values()), raising=False)
    monkeypatch.setattr(registry_mod.registry, "get_entry", lambda name, **kw: entries.get(name), raising=False)

    for preserve in (False, True):
        _mcp_agent.refresh_agent_mcp_tools(agent, preserve_prefix=preserve)
        assert agent.valid_tool_names == {"read_file", "mcp_ms365_list_events"}

    _mcp_agent.restore_agent_tool_prefix(agent, ["terminal", "read_file", "mcp_ms365_create_event"])
    assert "terminal" not in agent.valid_tool_names
    assert "mcp_ms365_create_event" not in agent.valid_tool_names
    assert {t["function"]["name"] for t in agent.tools} == agent.valid_tool_names
