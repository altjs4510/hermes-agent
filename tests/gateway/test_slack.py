...
class TestThreadReplyHandling:
    ...
    async def test_unaddressed_thread_message_carries_no_reply_directive(
        ...
        session_key = "agent:main:slack:group:T_TEAM:C123:123.000:U_USER"
        # suspended must be explicit: _has_active_session_for_thread ends in `not entry.suspended`,
        # and a bare MagicMock attribute is truthy → the thread would read as suspended.
        mock_session_store._entries = {session_key: MagicMock(suspended=False)}
    ...
    async def test_thread_reply_with_hermes_and_other_bot_mentions_still_responds(
        ...
        async def _users_info_side_effect(user=None, **_kwargs):
            # U_USER is the (human) sender — the sender-is-bot probes
            # (#35777 unlabeled-event heuristic, bot-to-bot re-trigger guard)
            # must not misclassify them. UOTHERBOT is the mentioned peer bot,
            # resolved by name-humanization below.
            if user == "U_USER":
                return {"user": {"is_bot": False, "is_app_user": False}}
            return {"user": {"is_bot": True, "is_app_user": True}}

        adapter_with_session_store._app.client.users_info = AsyncMock(
            side_effect=_users_info_side_effect
        )
        ...
        adapter_with_session_store.handle_message.assert_called_once()
        msg_event = adapter_with_session_store.handle_message.call_args[0][0]
        # Mention humanization (clean_content parity) now resolves the
        # remaining <@UOTHERBOT> token to a display name for the agent —
        # UOTHERBOT falls back to its own ID since the mock carries no
        # profile/real_name.
