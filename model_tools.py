...
def _emit_post_tool_call_hook(
    *, function_name: str, function_args: Dict[str, Any], result: Any,
    ...
    # Best-effort: never raises, never blocks the tool. See gateway/side_effect_audit.py.
    try:
        ...
        if _se is not None:
            ...
            if status is None:
                _audit_status, _audit_etype, _ = _tool_result_observer_fields(function_name, result)
            else:
            ...
            record_side_effect(
                tool_name=function_name, action_class=_action_class,
                source="cron:agent" if _is_cron else "agent", status=_audit_status or "unknown",
                actor=("cron:agent" if _is_cron else (_actor or None)), target_ref=_target_ref or None,
                args=function_args, result_preview=result, duration_ms=duration_ms or None,
                error_type=_audit_etype, task_id=task_id or None, tool_call_id=tool_call_id or None,
    ...
    # successful *read* tool. Best-effort; never raises, never blocks. Only read
    # tools taint (classify_read_source returns None for side effects).
    try:
        _se_status = status
        if _se_status is None:
            _se_status, _, _ = _tool_result_observer_fields(function_name, result)
...
def _pre_dispatch_guards(function_name: str, function_args: Dict[str, Any], skip_pre_tool_call_hook: bool,
    ...
    # Self-improvement change double-gate: during a Phase 2 re-dispatch the agent
    # runs as the owner (write-capable), so force an owner-confirm card before any
    # write_file/patch applies. No-op outside an exec turn. Fails closed on error.
    try:
        ...
        if si_block_message is not None:
            return function_args, (si_block_message, "self_improvement_gate", None)
    except Exception as _si_gate_err:
        logger.debug("self-improvement change gate error: %s", _si_gate_err)
        if function_name in {"write_file", "patch"}:
            return function_args, (json.dumps({"error": "자가발전 변경 게이트 실패로 적용 보류"}, ensure_ascii=False),
                                   "self_improvement_gate_error", None)
...
def _emit_egress_shadow_preflight(
    ...
    session_id: Optional[str] = None, tool_call_id: Optional[str] = None, turn_id: Optional[str] = None,
) -> None:
    """Egress lockdown preflight (Phase 1, SHADOW): record what the egress policy
    WOULD do for this side-effect tool given the session's content taint, then
    return — never blocks. Only egress-capability tools in a tainted session emit
    an event. See gateway/egress_policy.py. Best-effort: failures are swallowed."""
    try:
        ...
        _status = {
            "allow": "would_allow", "audit_only": "would_allow",
            "confirm": "would_confirm", "block": "would_block",
        }.get(decision.action, "would_allow")
        _sources = ",".join(sorted({e.source_class for e in taint.events})) or "unknown"
        record_side_effect(
            tool_name="egress_lockdown", action_class="egress_shadow", source="shadow", status=_status,
            actor=str(actor), target_ref=resource or None, task_id=task_id or None,
            tool_call_id=tool_call_id or None, turn_id=turn_id or None,
            rationale=(
...
def handle_function_call(
    ...
    try:
        ...
        # Egress lockdown preflight (Phase 1, SHADOW): records would_* for side-effect
        # tools in a content-tainted session, then always proceeds.
        _emit_egress_shadow_preflight(function_name, function_args, task_id=task_id, session_id=session_id,
