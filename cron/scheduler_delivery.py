...
def _deliver_result(
    ...
    # W1 side-effect audit ledger — no_agent cron deliveries bypass the tool executor (direct
    # _send_to_platform / live adapter), so the model_tools post-tool hook never sees them.
    ...
    try:
        ...
        record_side_effect(
            tool_name="_deliver_result", action_class="send",
            source=f"cron:{_audit_job_id}", actor=f"cron:{_audit_job_id}",
            status="failed" if delivery_errors else "success",
            target_ref=",".join(f"{t.get('platform')}:{t.get('chat_id')}" for t in targets) or None,
