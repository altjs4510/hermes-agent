...
_HERMES_CORE_TOOLS = [
    ...
    "text_to_speech",
    # Planning & memory. slack_list_todo: Cookie's Slack-List todo SoT (gated on token + list id).
    "todo_list", "slack_list_todo", "memory",
    "session_search",
    "clarify",
    "execute_code", "delegate_task",
    "cronjob_manage",
    # Cookie overlay: cross-platform messaging IS agent-callable here (gated on the
    # gateway running via check_fn) — the alter sends/edits on Cookie's behalf.
...
_CODING_TOOLS = _core_without("image_generate", "text_to_speech", "cronjob_manage", "computer_use",
                              "send_message", "update_message", *_HA_TOOLS, kanban=False)
...
TOOLSETS = {
    ...
    "todo": _ts("Task planning and tracking for multi-step work", ["todo_list", "slack_list_todo"]),
    "messaging": _ts(
        "Cross-platform messaging: send (and edit) messages on Telegram, Discord, Slack, SMS, etc.",
        ["send_message", "update_message"],
    ),
    ...
    # upstream has no agent-callable send_message — this deployment registers
    # send_message/update_message (Cookie overlay), but keeps them out of the
    # coding/ACP/API-server postures. hermes-acp is the coding posture minus the
    # interactive clarify UI.
    "hermes-acp": _ts(
    ...
    "hermes-api-server": _ts(
        ...
        _core_without("text_to_speech", "clarify", "computer_use", "send_message", "update_message", kanban=False),
    ),
